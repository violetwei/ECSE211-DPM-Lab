 ...
 *
 */
public class NavigationAvoidance extends Thread {
	 ...
	 * @param usData
	 */
	public NavigationAvoidance(SampleProvider us, float[] usData){
	...
	public void travelTo(double x, double y) {
		...
		for (EV3LargeRegulatedMotor motor : new EV3LargeRegulatedMotor[] {Lab3.getLeftMotor(), Lab3.getRightMotor()}) {
			motor.stop();
			motor.setAcceleration(500); //low acceleration to reduce uncertainty as the motors start up
		}
		...
		while (Lab3.getLeftMotor().isMoving() || Lab3.getRightMotor().isMoving()) { // Scan around when moving
			...
			if(distance <= bandCenter){
				...
				navigating = false; //quit navigation mode
			}
	...
	public void avoidBlock(){
		...
		if(!turning_right) { //turns left to avoid
